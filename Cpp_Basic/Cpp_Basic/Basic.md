...
### 생성자
 ...
 # 오버 라이딩
  ...
  // 함수를 외우는 것이 아니라 전반적인 흐름을 알아야 라이브러리 잘 찾을 수 있음
  // 어떤 툴을 사용하던간에 (언리얼, 유니티 등) 제공하는 함수+클래스 객체(=api) 기능을 가져다쓰게됨 
...
# 다형성
    ...
    - 오버로딩(Onverloading), 오버라이딩(Overriding)
...
# 순수 가상함수
    ...
    - 순수 가상함수가 있는 부모 클래스를 상속받은 자식 클래스는 반드시

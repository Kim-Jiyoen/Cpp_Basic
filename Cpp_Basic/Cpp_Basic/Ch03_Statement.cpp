...
void StatementIf() 
{
  ...
  // 총점이 90이상이면 "수", 80 이상이면 "우", 아니면 "재수강"
  int Total3 = 85;
  if (Total3 >= 90) {
    cout << "수" << endl;
  } else if (Total3 >= 80) {
  ...
  // 3과목 평균점수가 60 이상이면 합격, 아니면 불합격
  // 각 과목 당 점수가 60 미만이면 과락
  int Subject1 = 90;
  int Subject2 = 80;
  int Subject3 = 50;
  int Average = (Subject1 + Subject2 + Subject3) / 3;
  
  // 평균, 과목 당 점수가 모두 60 이상이면 합격
  ...
  int SubjectNumber = 0; // 이름 길게 쓰는 연습
  cout << "과목 번호를 입력해 주세요: ";
  cin >> SubjectNumber;
  ...
  // 같은지 비교해서 결과 출력하는 것은 swich로 바꿀 수 있음
  // switch->if는 대체로 가능, if->switch는 대체로 불가능
}
...
void StatementFor()
{
  ...
  // 변수 i가 for문 안에 있기 때문에 다른 블록에서도 i 사용 가능함.
  ...
  // 1차원 -> 선형, 2차원 -> 평면, 3차원 -> 입체, 4차원 -> Tensor
